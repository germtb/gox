...
func V(value any) VNode {
	...
	default:
		...
		panic(fmt.Sprintf("gox: cannot convert %T to VNode - use runtime.Text() for strings or return a VNode from your expression", value))
	}
}
...
// Useful for conditional rendering: {runtime.When(showExtra, <Extra />)}
...
// Useful for rendering lists: {runtime.Map(items, func(item Item) VNode { return <ItemView item={item} /> })}
...
// Useful for spread props: runtime.MergeProps(baseProps, spreadProps, extraProps)
