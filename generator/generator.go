...
type Options struct {
	// RuntimePackage is the import path for the runtime package.
	// Default: "github.com/germtb/gox/runtime"
...
func New(opts *Options) *Generator {
	g := &Generator{
		...
		runtimePkg: "github.com/germtb/gox/runtime",
	}
...
// Output: runtime.Element("tag", runtime.Props{...}, child1, child2, ...)
func (g *Generator) generateIntrinsicElement(elem *ast.JSXElement) {
	g.write("runtime.Element(")
...
func (g *Generator) generateProps(attrs []ast.Attribute) {
	...
	if hasSpread {
		// Use runtime.MergeProps for spread handling
		g.generatePropsWithSpread(attrs)
		return
	}

	g.write("runtime.Props{")
...
func (g *Generator) generatePropsWithSpread(attrs []ast.Attribute) {
	// Generate runtime.MergeProps(...) with props in order
	// This preserves the JSX semantics where later props override earlier ones

	g.write("runtime.MergeProps(")
...
func (g *Generator) generateJSXChild(child ast.JSXChild) {
	...
	case *ast.JSXText:
		...
		g.write(fmt.Sprintf("runtime.Text(%q)", text))
	...
	case *ast.JSXExpression:
		...
		if idx := strings.Index(transformed, " && "); idx != -1 {
			...
			g.write(fmt.Sprintf("runtime.When(%s, %s)", cond, rest))
		} else {
			// Wrap expressions in runtime.V() to convert any value to VNode
			g.write(fmt.Sprintf("runtime.V(%s)", transformed))
		}
